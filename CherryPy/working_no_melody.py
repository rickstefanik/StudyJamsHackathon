...
class MelodyController:
    def shiftPitch(self, newFile, shift):
        sound1 = AudioSegment.from_wav("temp.wav")
        # sound2 would be our chord
        sound2 = AudioSegment.from_wav("StudyLoopFinal.wav")
 
        output = sound1.overlay(sound2, loop=True)
        output.export("mixed_sounds.wav", format="wav")

        wr = wave.open('mixed_sounds.wav', 'r')
    ...
    def generateStartTag(self, pitch):
        ...
        return "".join(['<voice-transformation timbre="Sunrise" timbre_extent="100%" breathiness="0%" type="Custom" pitch="', pitch ,'%" pitch_range="-100%" rate="-20%" glottal_tension="-100%" timbre="Sunrise">'])
    ...
    def watsonSpeak(self, textFromUser, pitch, filename="output.wav"):
        ...
        melody = ["F", "F", "D", "D", "C", "C", "F", "F", "F", "D", "D", "F"]
